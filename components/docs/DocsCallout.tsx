...
type CalloutType = "info" | "tip" | "warning" | "important" | "note";
...
const CONFIG: Record<CalloutType, { icon: React.ElementType; bg: string; border: string; text: string }> = {
  ...
  note:      { icon: Info,         bg: "bg-zinc-500/10",   border: "border-zinc-500/30",  text: "text-zinc-400" },
...
export function DocsCallout({ type = "info", children }: { type?: string; children: React.ReactNode }) {
  const config = CONFIG[type as CalloutType] || CONFIG.info;
  const { icon: Icon, bg, border, text } = config;
